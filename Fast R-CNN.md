...
​       图Fig 2展示了，如何使用来自VGG16的fc6层中的25088×4096矩阵的顶部256个奇异值和来自fc7层的4096×4096矩阵的顶部256个奇异值减少运行时间，mAP只有少许损失。如果在压缩之后再次微调，则可以在mAP更小的下降的情况下进一步加速。

### 4.5. 精调哪些层

![fig1](pic/Fast_R-CNN-tab5.jpg)

​        对于在SPPnet论文[11]中考虑的不太深的网络，仅微调全连接层似乎足以获得良好的精度。我们假设这个结果不适用于非常深的网络。为了验证微调卷积层对于VGG16的重要性，我们使用Fast R-CNN微调，但冻结十三个卷积层，以便只有全连接层学习。这种消融模拟单尺寸SPPnet训练，将mAP从66.9％降低到61.4％（表5）。这个实验验证了我们的假设：通过RoI池化层的训练对于非常深的网是重要的。
